...
// time-machine-ml-api.php
//
// Queue-based bridge between the Time Machine ML EA (MT5) and API clients.
//
// Flow:
//   Client  → GET  ?symbol=EURUSD&api_key=XXX  → creates .req.json, polls for .res.json
//   EA      → GET  (no params)                 → receives next .req.json
//   EA      → POST request_id + payload        → writes .res.json
//   Client  ← receives arrissa_data response
//
...
$debugFile = "{$baseDir}time-machine-ml-debug.log";
...
//--------------------------------------------------------------------
// 1) EA POST: receive built payload and store as response
//--------------------------------------------------------------------
...
//--------------------------------------------------------------------
// 2) Client GET with ?symbol=: enqueue request and wait for response
//--------------------------------------------------------------------
...
if ($symbol) {
    ...
    debug_log("Client GET enqueued request_id=$request_id for symbol=$symbol");

    $start   = time();
    $timeout = 10; // seconds to wait for EA response
    ...
    echo json_encode(['arrissa_data' => [
        ...
        'message' => 'No Expert Advisor is currently running to process this request. Attach the TimeMachine ML API EA in your MT5 terminal.',
    ]]);
    exit;
}
...
echo json_encode(['arrissa_data' => [
    'status'  => 'polling',
    'message' => 'No pending Time Machine ML requests',
]]);
exit;
