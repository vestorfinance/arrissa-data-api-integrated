...
// trend-bias-api.php
//
// Queue-based bridge between the Trend Bias EA (MT5) and API clients.
//
// Flow:
//   Client → GET  ?symbol=EURUSD&timeframe=D1&api_key=XXX
//          → creates .req.json, polls for .res.json
//   EA     → GET  (no params)              → receives next .req.json
//   EA     → POST request_id + payload     → writes .res.json
//   Client ← receives arrissa_data with bias, confirmed_swing_price/time/type
//
...
$debugFile = "{$baseDir}trend-bias-debug.log";
...
//--------------------------------------------------------------------
// 1) EA POST: receive bias payload and store as response
//--------------------------------------------------------------------
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$request_id || !$payload) {
        ...
        debug_log("EA POST missing request_id or payload");
...
if ($symbol) {
    ...
    $timeframe = strtoupper(trim($_GET['timeframe'] ?? ''));
    ...
    if (!in_array($timeframe, $validTimeframes)) {
        ...
        echo json_encode(['arrissa_data' => [
            ...
            'message' => 'timeframe must be one of: ' . implode(', ', $validTimeframes),
        ]]);
        exit;
    }
    ...
    debug_log("Client GET enqueued request_id=$request_id symbol=$symbol timeframe=$timeframe");
    ...
    echo json_encode(['arrissa_data' => [
        ...
        'message' => 'No Expert Advisor is currently running to process this request. Attach the Trend Bias API EA in your MT5 terminal.',
    ]]);
    exit;
}
...
echo json_encode(['arrissa_data' => [
    'status'  => 'polling',
    'message' => 'No pending Trend Bias requests',
]]);
exit;
